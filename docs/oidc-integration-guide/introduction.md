...
See [OpenID Connect FAQ](https://openid.net/connect/faq/) for a set of answers to Frequently Asked Questions.
:::
...
- **OpenID Connect Provider (OP)** is an authorization server that offers authentication as a service and provides claims to a Relying Party about the authentication event and the End-User.
...
::: warning
A Relying Party must be authorized to use a Refresh Token.
:::
